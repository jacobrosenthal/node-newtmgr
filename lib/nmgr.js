...
const BLE_SEQ_MIN = 0
const BLE_SEQ_EVT_MIN = 255

var nextSeq = BLE_SEQ_MIN;
...
function generateTaskStatsBuffer()
{
  var nmr = {};
  nmr.Data = Buffer.alloc(0);
  nmr.Op = CONSTANTS.NMGR_OP_READ;
  ...
  nmr.Seq = getNextSeq();
  nmr.Id = CONSTANTS.NMGR_ID_TASKSTATS;

  return _serialize(nmr);
}

function generateMPStatsBuffer()
{
  var nmr = {};
  nmr.Data = Buffer.alloc(0);
  nmr.Op = CONSTANTS.NMGR_OP_READ;
  ...
  nmr.Group = CONSTANTS.NMGR_GROUP_ID_DEFAULT;
  nmr.Seq = getNextSeq();
  nmr.Id = CONSTANTS.NMGR_ID_MPSTATS;
...
function generateImageEraseBuffer(){
  var nmr = {};
  nmr.Data = Buffer.alloc(0);
  nmr.Op = CONSTANTS.NMGR_OP_WRITE;
  nmr.Flags = 0;
  ...
  nmr.Group = CONSTANTS.NMGR_GROUP_ID_IMAGE;
  nmr.Seq = getNextSeq();
  nmr.Id = CONSTANTS.IMGMGR_NMGR_ID_ERASE;
  ...
  function transform(data, enc, cb) {
    ...
    if(typeof header !== 'undefined'){
      if(nonmgrhdr){
        ...
        debug("append, header data lenght: ", header.Data.length);
      }
...
module.exports = {
  generateTaskStatsBuffer: generateTaskStatsBuffer,
  generateMPStatsBuffer: generateMPStatsBuffer,
  ...
  generateImageEraseBuffer: generateImageEraseBuffer,
